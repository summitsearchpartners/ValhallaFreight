...
import ShipmentScheduleFields from '../../shared/freight/ShipmentScheduleFields';
...
export default function QuoteStudioPage(){
  ...
  async function rate(e:any){
    ...
    try{
      const r:any=await api('/quotes/rate',{method:'POST',body:JSON.stringify({
        ...
        requested_pickup_at:f.get('requested_pickup_at')||null,
        requested_delivery_at:f.get('requested_delivery_at')||null,
  ...
  return <>
    ...
    <div className="quotegrid">
      <Card><form onSubmit={rate}><div className="sectiontitle"><MapPin/><div><h3>Lane information</h3><p>Use a saved customer location or search any verified address.</p></div></div><label>Customer<select required value={selectedCustomerId} onChange={e=>setSelectedCustomerId(Number(e.target.value))}>{customers.map(c=><option value={c.id} key={c.id}>{c.name}</option>)}</select></label>{locationsLoading&&<LocationsLoading/>}<div className="form2 quoteLaneGrid"><LaneAddressSelector title="ORIGIN" address={origin} setAddress={setOrigin} locations={locations}/><LaneAddressSelector title="DESTINATION" address={destination} setAddress={setDestination} locations={locations}/></div><ShipmentScheduleFields/><FreightDetailsFields/>{error&&<div className="inlineError">{error}</div>}<button className="btn primary wide" disabled={loading||!selectedCustomerId}><Sparkles size={17}/>{loading?'Rating network...':'Get live rates'}</button></form></Card>
