...
export function PageHead({eyebrow='FreightForge',title,sub,actions}:{eyebrow?:string,title:string,sub:string,actions?:ReactNode}){return <div className="pagehead"><div><span className="eyebrow">{eyebrow}</span><h1>{title}</h1><p>{sub}</p></div>{actions&&<div className="actions">{actions}</div>}</div>}
