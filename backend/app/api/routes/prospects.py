...
async def analyze(file: UploadFile = File(...)):
    ...
    return {"shipments":shipments,"annual_spend":round(annual_spend,2),"avg_spend_per_shipment":round(annual_spend/shipments,2) if shipments else 0,"avg_weight":round(avg_weight,1),"columns":list(df.columns),"message":"FreightForge prospect ingestion is active. Map additional source columns to unlock lane, class, carrier, accessorial and savings analysis."}
